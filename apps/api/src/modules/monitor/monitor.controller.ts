...
import { getMonitorStatus, toggleMonitored, toggleIpMonitored } from './monitor.service.js';
...
export async function toggleIpMonitor(req: Request, res: Response): Promise<void> {
  const { monitored } = z.object({ monitored: z.boolean() }).parse(req.body);
  const id = String(req.params['id'] ?? '');
  if (!id) throw new AppError(400, 'IP address ID required');
  await toggleIpMonitored(id, monitored);
  res.status(204).send();
}
